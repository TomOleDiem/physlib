...
# Convex structure and purity of states

Evaluation positivity/reality facts, convex mixtures (`mix`), purity (`IsPure`), and finite
convex mixtures (`finiteMix`) for `OperatorAlgebra.State` (`States/Basic.lean`). Ported from a
separate staging tree's trimmed `Physlib.Mathematics.OperatorAlgebra.State` -- only what
`FiniteDim.StateRepresentation`/`FiniteDim.PureStates` need. Probabilities of effects and
pullback of states along star-homomorphisms are not included.
