...
The oscillator's headline results, one after another, with no new proofs — each lemma here is a
direct citation of a result proved elsewhere in this tree (`DifferentialCore.lean`,
...
This is the `d = 1` case of the spectrum/eigenstate TODOs in
`HarmonicOscillator/Basic.lean`, proved for `Physlib.QuantumMechanics.HarmonicOscillator.
OneDimension.HarmonicOscillator` (`OldOscillator` above) rather than the `d`-dimensional
`HarmonicOscillator d` that file and `LadderOperators.lean` use — the two formalizations are not
yet connected.
-/
...
lemma summary_eigenvalue_formula (n : ℕ) :
...
lemma summary_eigenfunction_formula (n : ℕ) (x : Space 1) :
...
lemma summary_selfAdjoint : IsSelfAdjoint (differentialHamiltonianClosure q) :=
...
lemma summary_eigenfunction_is_eigenvector (n : ℕ) :
...
lemma summary_energy_levels (E : ℝ) :
...
every time `t`. -/
lemma summary_stone_time_evolution (t : ℝ) :
