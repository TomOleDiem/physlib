...
`HarmonicOscillator.EssentialSelfAdjointness` proves `Q.hamiltonian` is essentially self-adjoint
and builds the `QuantumSystem` generated by its self-adjoint closure, using the reusable
defect-index/spectral infrastructure together with a model-specific analytic certificate. Both
results live there rather than here because the proof needs that reusable machinery, which this
