...
lemma rawValue_indep (hw : w.IsFinite) (A : E) {r s : ℝ} (hr : 0 ≤ r • (1 : E) + A)
    (hs : 0 ≤ s • (1 : E) + A) : rawValue hw A r hr = rawValue hw A s hs := by
...
lemma toFun_eq (hw : w.IsFinite) (A : E) {r : ℝ} (h : 0 ≤ r • (1 : E) + A) :
    toFun hw A = rawValue hw A r h :=
  rawValue_indep hw A _ h

@[simp]
lemma toFun_of_nonneg (hw : w.IsFinite) (A : PosCone E) : toFun hw (A : E) = (w A).toReal := by
...
lemma toFun_neg (hw : w.IsFinite) (A : E) : toFun hw (-A) = -toFun hw A := by
...
lemma toFun_smul (hw : w.IsFinite) (t : ℝ) (A : E) : toFun hw (t • A) = t * toFun hw A := by
  rcases le_total (0 : ℝ) t with ht | ht
  · exact toFun_real_nonneg_smul hw ht A
  · have h1 : t • A = -((-t) • A) := by rw [neg_smul, neg_neg]
    rw [h1, toFun_neg, toFun_real_nonneg_smul hw (neg_nonneg.mpr ht) A]
    ring
...
@[simp]
lemma toLinearMap_apply (hw : w.IsFinite) (A : E) : toLinearMap hw A = toFun hw A := rfl
