...
public import Physlib.QuantumMechanics.OperatorAlgebra.WStarAlgebra
...
# W⋆ measurement primitives: projection-valued measures
...
`NormalState` itself used to live in this file, as a cheap stand-in (`State M` repackaged with no
continuity condition imposed) pending the predual/weak-⋆ topology project `OperatorAlgebra/todo.md`
flagged as a prerequisite (2026-08-24). That project now exists (`WStarAlgebra.lean`) — `NormalState`
has moved there, defined for real as a weak-⋆-continuous state, and this file now only re-exports it
via that import.
