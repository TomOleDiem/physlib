...
to the real spectral measure). Unlike the bounded case (`Measurement/State.lean`), an unbounded
...
in the state `ω`. Mirrors `Measurement/State.lean`'s `State.expectation` for bounded observables,
