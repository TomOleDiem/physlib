...
`boundedTransform` is recorded honestly as `@[sorryful]`.
-/
...
TODO "Prove `AffiliatedObservable.boundedTransform_ofBoundedTransform`. Needs \
  `a = S.boundedFC id` for `S = BorelFunctionalCalculus.spectralMeasure ⟨a, ha⟩`, i.e. that the \
  supplied Borel calculus's spectral measure really integrates the identity back to `a` — the \
  same deferred compatibility `Affiliated.lean` documents at `Observable.toAffiliatedObservable` \
  ('The bounded inclusion recovers `a` itself as the affiliated operator's spectral integral of \
  the identity'), not a new gap. Given that one compatibility fact, this theorem follows from \
  `boundedFC_measurableRealFC` and `measurableRealFC_tanh_measurableRealFC_artanh` exactly as \
  `boundedTransform_eq_boundedFC_measurableRealFC_tanh` does above."
