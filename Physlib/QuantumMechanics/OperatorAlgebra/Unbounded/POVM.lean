...
`todo.md` investigated and chose between two designs for a genuinely general (not merely
projection-valued) positive-operator-valued measure; the abstract W⋆-algebra route (its
"Alternative" section) is what `PVM`/`NormalState` (`NormalState.lean`, née the file this
investigation was written in) already implement for the sharp/projective case. This file lifts
that same construction from projections to general effects, which is the textbook notion of a
POVM (`M(S) ≥ 0`, `M(X) = 1`, valued in an additive operator space, *not* restricted to
...
- `FinitePOVM.toPOVM` : the finite-discrete case (`Basic.lean`'s `FinitePOVM`, viewed on the
...
Generalizes `PVM X A` (`NormalState.lean`) by dropping the requirement that every value be a star
...
/-- **The finite-discrete case recovers a `POVM`**: `Basic.lean`'s `FinitePOVM A X` (a family of
