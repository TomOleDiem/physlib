...
- [ ] **Construct the qubit algebra from generators and relations** (not started; the genuine
      fix for the `pauliBasis` question above). Build `P := FreeAlgebra ℂ (Fin 3)` quotiented by
      the two-sided ideal generated by the Pauli relations (`X² = 1`, `XY = iZ`, cyclically),
      then *prove* — not assume — that this specific presented algebra is unital, four-dimensional,
      and `1, X, Y, Z` a basis of it, giving a genuine `QubitAlgebra` instance with `pauliBasis`
      as a theorem rather than a hypothesis. Sketch: spanning by an induction that rewrites any
      monomial in the generators down to a `ℂ`-combination of `1, X, Y, Z` using the relations;
      independence by an algebra map from `P` to `Matrix (Fin 2) (Fin 2) ℂ` sending `X, Y, Z` to
      the actual Pauli matrices (exists by the universal property of the quotient, since the
      matrices satisfy the same relations) and showing that composite is injective on the
      spanning set. Using matrices *here* is fine and different in kind from the earlier
      `SO(3)`/`{±1}`-kernel correction: there the point was that `SO(3)` structure on `A` itself
      must stay representation-free; here the matrices are only a proof device pinning down the
      size of one concrete, freely-constructed algebra, not part of any definition on a general
      `A`. Doesn't change `QubitAlgebra`'s fields — it's an existence witness (the class is
      satisfiable, and this is close to its canonical/minimal instance), not a replacement for
      them; an arbitrary `QubitAlgebra A` can still be a strictly larger algebra.
