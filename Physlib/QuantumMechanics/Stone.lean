...
* `Matrix.UnitaryOneParameterSubgroup.ofHermitian`: The converse direction -- every Hermitian
    matrix generates a continuous unitary group in the convention `U(t) = exp (-itH)`.
...
/-- Every anti-Hermitian matrix generates a continuous unitary one-parameter subgroup
`t ↦ exp (tA)`. -/
...
/-- Every Hermitian matrix generates a continuous unitary one-parameter subgroup
`t ↦ exp (-itH)`. -/
...
def ofHamiltonian {H : Matrix (Fin n) (Fin n) ℂ} (hH : IsSelfAdjoint H) :
    Matrix.UnitaryOneParameterSubgroup n :=
  ofHermitian (H := (ℏ : ℂ)⁻¹ • H) (by
    apply IsSelfAdjoint.smul
    · exact (by
        rw [isSelfAdjoint_iff, Complex.star_def, Complex.conj_ofReal] :
          IsSelfAdjoint (ℏ : ℂ)).inv₀
    · exact hH)
...
@[simp] lemma value_ofHamiltonian {H : Matrix (Fin n) (Fin n) ℂ} (hH : IsSelfAdjoint H)
  ...
  rw [ofHamiltonian, value_ofHermitian]
  rw [smul_smul]
  field_simp [Constants.ℏ_ne_zero]
